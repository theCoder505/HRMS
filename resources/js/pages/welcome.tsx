        ...
        <>
            ...
            <nav className="nav">
                ...
                <div className="nav-actions">
                    {auth.user ? (
                        ...
                        <Link href={route('employee.dashboard')} className="btn btn-primary">
                            Employee Dashboard
                        </Link>
                    ) : (
                        <>
                            <Link href={route('employee.login')} className="btn btn-ghost">
                                Employee Portal
                            </Link>
                            <Link href={route('hrm.login')} className="btn btn-primary">
                                HR Login
                            </Link>
                        </>
                    )}
            ...
            <div className="hero-wrap">
                ...
                <h1 className="hero-title fu d1">
                    ...
                    <br />
                    for your <span className="grad">people team</span>
                </h1>
                <p className="hero-sub fu d2">
                    Payroll, attendance, leaves, performance and more — unified in one beautiful, fast platform that your team will actually love to
                    use.
                </p>
                <div className="hero-actions fu d3">
                    {auth.user ? (
                        ...
                        <>
                            ...
                            <Link href={route('hrm.login')} className="btn btn-ghost btn-lg">
                                HR Login
                            </Link>
                        </>
                    )}
                ...
                <div className="portals">
                    <Link href={route('employee.login')} className="portal-card emp">
                        ...
                        <div className="portal-desc">
                            Access your salary slips, attendance logs, leave requests, announcements and more from one place.
                        </div>
                    ...
                    <Link href={route('hrm.login')} className="portal-card">
                        ...
                        <div className="portal-desc">
                            Manage employees, run payroll, approve leaves, track attendance and configure your organization.
                        </div>
                    ...
                    {[
                        {
                            ...
                            desc: 'Centralize employee records, org charts, and team structures in one elegant hub.',
                        },
                        {
                            icon: 'M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6',
                            title: 'Payroll & Benefits',
                            desc: 'Automate salary runs, deductions and bonus releases without the spreadsheet chaos.',
                        },
                        {
                            icon: 'M3 4h18v18H3zM16 2v4M8 2v4M3 10h18',
                            title: 'Leave & Attendance',
                            desc: 'Track time-off requests, shift schedules, and attendance with smart conflict detection.',
                        },
                        {
                            icon: 'M18 20V10M12 20V4M6 20v-6',
                            title: 'Performance Reviews',
                            desc: 'Run feedback cycles and tracking that employees actually look forward to.',
                        },
                        {
                            icon: 'M12 2l7 4v6c0 5-3.5 9.74-7 11-3.5-1.26-7-6-7-11V6z',
                            title: 'Compliance & Audit',
                            desc: 'Stay ahead of labour laws with built-in policy templates and real-time audit trails.',
                        },
                        {
                            icon: 'M13 2L3 14h9l-1 8 10-12h-9l1-8z',
                            title: 'Instant Onboarding',
                            desc: 'Cut day-one admin from hours to minutes with automated checklists.',
                        },
                    ].map(({ icon, title, desc }) => (
                    ...
                    {[
                        {
                            ...
                            body: 'PeopleOS replaced four separate tools. Our HR team went from firefighting to actually building culture.',
                        },
                        {
                            name: 'Carlos Reyes',
                            role: 'COO, Meridian Logistics',
                            body: "Payroll used to take two full days. Now it's a Tuesday-morning click. I can't overstate how much that matters.",
                        },
                        {
                            name: 'Priya Nair',
                            role: 'HR Director, Bloom Health',
                            body: 'The compliance module alone saved us from a very expensive audit. Worth every penny and then some.',
                        },
                    ].map(({ name, role, body }) => (
            ...
            <footer>
                <div className="flex w-full flex-col items-center justify-between gap-8 md:flex-row">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden gap-6 border-l border-white/10 pl-6 md:flex">
                            <Link href={route('about')} className="text-[13px] text-[#8b8fa8] transition-colors hover:text-white">
                                About Us
                            </Link>
                            <Link href={route('privacy.policy')} className="text-[13px] text-[#8b8fa8] transition-colors hover:text-white">
                                Privacy Policy
                            </Link>
                            <Link href={route('terms.conditions')} className="text-[13px] text-[#8b8fa8] transition-colors hover:text-white">
                                Terms & Conditions
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-2 md:items-end">
                        {settings?.contact_email && (
                            <a href={`mailto:${settings.contact_email}`} className="text-[12px] text-[#8b8fa8] transition-colors hover:text-white">
                                {settings.contact_email}
                            </a>
                        )}
                        <div className="flex gap-4">
                            {settings?.facebook && (
                                <a href={settings.facebook} target="_blank" className="text-[#8b8fa8] transition-colors hover:text-white">
                                    <svg
                                        width="16"
                                        height="16"
                                        viewBox="0 0 24 24"
                            ...
                            {settings?.twitter && (
                                <a href={settings.twitter} target="_blank" className="text-[#8b8fa8] transition-colors hover:text-white">
                                    <svg
                                        width="16"
                                        height="16"
                                        viewBox="0 0 24 24"
                            ...
                            {settings?.instagram && (
                                <a href={settings.instagram} target="_blank" className="text-[#8b8fa8] transition-colors hover:text-white">
                                    <svg
                                        width="16"
                                        height="16"
                                        viewBox="0 0 24 24"
                            ...
                            {settings?.linkedin && (
                                <a href={settings.linkedin} target="_blank" className="text-[#8b8fa8] transition-colors hover:text-white">
                                    <svg
                                        width="16"
                                        height="16"
                                        viewBox="0 0 24 24"
